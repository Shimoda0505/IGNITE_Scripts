...
public class CameraController : MonoBehaviour
{
    ...
    #region 追従移動
    [Header("移動")]
    [SerializeField, Tooltip("追従対象")] private Transform _targetPos;
    [SerializeField, Tooltip("ターゲットとカメラの離す距離")] private float _toTargetDistance;
    private Vector3 _toTarget = default;//ターゲットとカメラの現在の距離
    [SerializeField, Tooltip("カメラの追従速度")] private float _followingSpeed;
    private Vector3 _defaultPos = default;//カメラの初期位置
    #endregion


    #region 入力移動
    [SerializeField, Tooltip("移動先の位置")] private Vector2 _moveClamp;
    private Vector3 _movePos = default;//カメラの移動位置
    #endregion


    #region 角度
    [Header("角度")]
    [SerializeField, Tooltip("角度の移動先の最大値")] private Vector3 _rotateClamp;
    [SerializeField, Tooltip("追従時のカメラの回転速度")] private float _followingRotateSpeed;
    private Quaternion _defaultRoatete = default;//追従時のカメラの角度のみ計算
    private Vector3 _cameraRotate = default;//カメラ角度の代入
    private const int _angleToRotate = 180;//AngleをRotateに変換
    #endregion


    #region 回避
    [Header("回避")]
    [SerializeField, Tooltip("回避時のカメラ移動")] private float _doageMovePos;
    #endregion
