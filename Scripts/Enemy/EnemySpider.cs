...
public class EnemySpider : MonoBehaviour
{
    ...
    #region スクリプト
    [Header("スクリプト")]
    [SerializeField, Tooltip("プレイヤーのルートナビ")] RootNav _rootNav;
    [SerializeField, Tooltip("プール管理")] PoolManager _poolManager;
    PoolController _poolController;//蜘蛛の糸
    EnemyCameraView _enemyCameraView;//カメラ描画スクリプト
    #endregion

    #region 発射管理
    [Header("発射")]
    [SerializeField, Tooltip("プレイヤー")] private GameObject _player;
    [SerializeField, Tooltip("弾発射位置")] private GameObject _shotPos;
    [SerializeField, Tooltip("プレイヤーとの距離")] private float _activeDistance;
    private GameObject _bulletObj = default;//発射syry弾のオブジェクト
    private string _spiderBoll = "蜘蛛の糸";//プールから使用する弾オブジェクトの名前
    #endregion

    #region 時間
    [Header("時間")]
    [SerializeField, Tooltip("移動からの消えるまでの時間")] private float _activeFalseTime;
    private float _count = 0;//時間計測
    #endregion

    #region その他
    [Header("その他")]
    [SerializeField, Tooltip("アクティブポイント")] private GameObject _movePos;
    private Animation _anim;//アニメーター
    #endregion

    private Motion _motion = Motion.WAIT;//行動enum
    private enum Motion
    {
    ...
    private void Setting()
    {
        ...
        for (int i = 0; i <= _poolManager._poolArrays.Length - 1; i++)
        {
            ...
            //名前一致のプールを探索かける
            if (poolName == _spiderBoll) { _poolController = poolScript; }
        }
    }
    ...
    /// 死亡時の落下
    /// </summary>
    private void Death() { transform.Translate(Vector3.down); }
