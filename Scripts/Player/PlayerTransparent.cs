...
public class PlayerTransparent : MonoBehaviour
{
    ...
    #region カーソル
    [Header("カーソル関連")]
    [SerializeField, Tooltip("CursorTransform")] private Transform _cursorPos;
    [SerializeField, Tooltip("カーソル距離")] private float _distance;
    [SerializeField, Tooltip("メインカメラ")] private Camera _mainCamera;
    ...
    #region マテリアル
    [Header("マテリアル関連")]
    [SerializeField, Tooltip("メッシュ")] private SkinnedMeshRenderer _mesh;
    [SerializeField, Tooltip("透過マテリアル")] private Material[] _transMaterials;
    [SerializeField, Tooltip("非透過マテリアル")] private Material[] _opaMaterials;
    [SerializeField, Tooltip("透過マテリアル(ULT)")] private Material[] _transMaterialsUlt;
    [SerializeField, Tooltip("非透過マテリアル(ULT)")] private Material[] _opaMaterialsUlt;
    private bool _isUlt = false;//ウルト中かどうか
    #endregion

    #region ディゾルブ
    [Header("ディゾルブ関連")]
    [SerializeField, Tooltip("差し替え速度")] private float _changeDisSpeed;
    private int _dis = 0;//ディゾルブID
    private float _disMove = 0;//ディゾルブの現在値
    private bool _isDis = false;//差し替え
    #endregion

    #region ブーリアン
    [Header("ブーリアン関連")]
    [SerializeField, Tooltip("ブーリアン速度")] private float _changeBospeed;
    private int _bo;//ブーリアンID
    private int _rim;//リムパワーID
    [SerializeField,Tooltip("ブーリアンの現在値")] private float _rimMove;
    private Vector2 _rimClamp = new Vector2(1, 0.4f);//ブーリアンの最大最小
    #endregion

    #region エフェクト
    [Header("エフェクト")]
    [SerializeField, Tooltip("赤炎")] private GameObject _redFire;
    [SerializeField, Tooltip("青炎")] private GameObject _blueFire;
    #endregion


    Motion _motion = Motion.OPAQUA;//トランスペアレントの状態
    enum Motion
    {
        CHANGE,
        TRANSPARENT,
