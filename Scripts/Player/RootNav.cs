...
public class RootNav : MonoBehaviour
{
    ...
    #region 外部参照
    private GameObject _nowPoint = default;//現在のポイント位置を格納
    /// <summary>
    /// //現在のポイント位置
    /// </summary>
    ...
    #region イベントポイント関連
    [Header("イベントポイント関連")]
    [SerializeField, Tooltip("イベントポイントの親を設定")] private GameObject _playerRootPoints;
    [SerializeField, Tooltip("イベントポイントの配列")] private List<GameObject> _rootPoints;
    [SerializeField, Tooltip("ゴール地点との冗長距離")] private float _distansDistance;
    [SerializeField, Tooltip("ループ地点の配列番号")] private int _loopNumber;
    [SerializeField, Tooltip("ボス本体")] private GameObject _bossObj;
    [SerializeField, Tooltip("ボスのアクティブ位置")] private int _bossActiveNumber;
    private int _posNumber = 0;//現在位置のポイント番号
    #endregion
    ...
    /// イベントの起こるポイントを配列に格納
    /// </summary>
    private void RootArrayStoring()
    {

        //ボスのアクティブを消す
        for (int i = 0; i <= _bossObj.transform.childCount - 1; i++) { _bossObj.transform.GetChild(i).gameObject.SetActive(false); }
