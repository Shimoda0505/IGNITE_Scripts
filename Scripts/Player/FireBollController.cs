...
public class FireBollController : MonoBehaviour
{
    ...
    #region 取得関連
    private GameObject _targetObj;//追従する対象
    PointerLockOn _pointerLockOn;//ロックオンのスクリプト
    PlayerLinkEnemy _playerLinkEnemy;//敵格納の配列スクリプト    
    ...
    [SerializeField, Tooltip("発射時の旋回速度")] private float rotateSpeedStart;
    [SerializeField, Tooltip("旋回速度")] private float rotateSpeed;
    #endregion


    #region 時間関連
    [SerializeField, Tooltip("ロックオン対象がいない時の消えるまでの時間")] private float _disappearTime;
    private float _disappearCount;//ロックオン対象がいない時の消えるまでの時間を計測
    [SerializeField, Tooltip("旋回速度切り替わり時間")] private float _changeTime;
    private float _changeCount = 0;//旋回速度切り替わり時間を計測
    #endregion

    
    private const int ATTACK_DAMAGE = 100;//火球の火力
    
    private int _lockOnCount;//ロックオン数
