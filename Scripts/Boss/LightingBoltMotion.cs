...
public class LightingBoltMotion : MonoBehaviour
{
    ...
    private class BoltDatas
    {
        ...
        [SerializeField, Tooltip("落雷インターバル")] public float _interval;
        [SerializeField, Tooltip("落雷ポイント")] public GameObject _point;
        [SerializeField, Tooltip("落雷")] public GameObject[] _bolt;
    }
    [SerializeField,Tooltip("落雷データのリスト")] private List<BoltDatas> _boltDatas = new List<BoltDatas>();
    private float _count = 0;//時間のカウント
    private int _numberDatas = 0;//配列番号
    private int _numberBolt = 0;//落雷番号
    private GameObject _lastPoint = null;//前回のポイント
    private Motion _motion = Motion.WAIT;//挙動enum
    private enum Motion
    {
        WAIT,
        MOVE
