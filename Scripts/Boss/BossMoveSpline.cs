...
public class BossMoveSpline : MonoBehaviour
{
    ...
    [SerializeField,Tooltip("ボスのイベント管理スクリプト")] RizouSystem _rizouSystem;/*【他メンバーが制作したため添付してません】*/
    ...
    [SerializeField,Tooltip("ボスアニメーション管理スクリプト")] RaizoAnimationController _raizoAnim;
    [SerializeField,Tooltip("プレイヤー挙動管理スクリプト")] PlayerController _playerController;
    [SerializeField,Tooltip("プレイヤーステータス管理スクリプト")] PlayerStatus _playerStatus;
    [SerializeField,Tooltip("カメラ揺れスクリプト")] CameraShake _cameraShake;
    [SerializeField,Tooltip("ボスseスクリプト")] RaizoSe _raizoSe;
    #endregion

    #region プレイヤー関連
    [Header("プレイヤー関連")]
    [SerializeField, Tooltip("プレイヤーオブジェクト")] private GameObject _playerObj = default;
    #endregion

    #region 攻撃関連
    [Header("攻撃関連")]
    [SerializeField, Tooltip("突進の判定")] private float _tackleDis;
    private bool _istackle = default;//突進中かどうか
    [SerializeField, Tooltip("タックルエフェクト")] private GameObject _tackleEff;
    [SerializeField, Tooltip("カメラの振動時間/振動幅")] private Vector2 _shakeTimeMag;
    private int _tackleAttack = 250;//タックルの攻撃力
    #endregion

    #region スプライン関連
    private float _rootSpeed = default;//ルート速度
    private float _percentage = default;//補間の割合(0~1の間を始点^終点で移動)
    private Vector3 _prevPos = default;//前フレームのワールド位置
    private GameObject _lastPoint = default;//前回のNowRootを補完
    private SplineContainer _settingSpline = default;//設定されたスプライン  /*【SplineのEditor拡張が必要です】*/
    #endregion

    #region 行動情報
    [Header("行動情報")]
    [SerializeField, Tooltip("変更ポイントとの距離")] private float _pointDistance;
    private int _motionNumber = 0;//現在のモーションデータの番号
    private int _changeNumber = 0;//挙動変更番号
    [Serializable, Tooltip("ターゲット関連の格納要素")]
    public class MotionDatas
    {

        [SerializeField, Tooltip("プレイヤーの移動情報")] public GameObject _playerRoot;
        [SerializeField, Tooltip("雷蔵のスプライン情報")] public SplineContainer _moveSpline;/*【SplineのEditor拡張が必要です】*/
        [SerializeField, Tooltip("雷蔵の移動速度")] public float[] _moveSpeed;
        [SerializeField, Tooltip("雷蔵のモーション変化ポイント")] public GameObject[] _motionChange;
        [SerializeField, Tooltip("各ポイントのウルトモーション")] public ActiveMotion[] _activeMotion;
    ...
    public List<MotionDatas> _motionDatas = new List<MotionDatas>();//ターゲットのList
    #endregion
    ...
    private void FixedUpdate()
    {
        ...
        //イベント中 || プレイヤー死亡時は停止
        if (_gameSystem.IsEvent() || _playerController._playerMotion == PlayerController.PlayerMotion.Death) { return; }
    ...
    /// ギズモ表示
    /// </summary>
    private void OnDrawGizmos() { Gizmos.DrawWireSphere(this.gameObject.transform.position, _tackleDis); }
}
