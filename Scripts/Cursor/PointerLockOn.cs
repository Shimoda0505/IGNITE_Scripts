...
public class PointerLockOn : MonoBehaviour
{
    ...
    #region スクリプト
    [Header("スクリプト")]
    [SerializeField, Tooltip("ゲームのイベントやTime.deltaTimeを管理スクリプト")] GameSystem _gameSystem;
    [SerializeField, Tooltip("敵とプレイヤーの繋ぎを管理スクリプト")] PlayerLinkEnemy _playerLinkEnemy;
    [SerializeField, Tooltip("プレイヤーのプール管理スクリプト")] PoolManager _poolManager;
    [SerializeField, Tooltip("カーソル変更管理スクリプト")] PointerChange _pointerChange;
    PlayerSE _playerSe;//プレイヤーの音関連
    PlayerStatus _playerStatus;//プレイヤーのステータス
    PlayerController _playerController;//プレイヤーコントローラー
    PoolController _fireBollPool;//火球プール
    PoolController _fireBollUltPool;//火球(ウルト)プール  
    PoolController _explosionPool;//爆発プール
    PoolController _explosionUltPool;//爆発(特別)プール    
    ...
    #region ポインター関連
    [Header("ポインター関連")]
    [SerializeField, Tooltip("ポインターのロックオン範囲")] private Vector3 _pointerClamp;
    [SerializeField, Tooltip("ポインターのロックオン範囲(ウルト)")] private float _pointerClampUlt;
    [SerializeField, Tooltip("カーソルの親オブジェクト")] private GameObject _cursorObj;
    private float _pointerClampAssignment;//ポインターのロックオン範囲の代入
    private List<GameObject> _cursorObjs = new List<GameObject>();//カーソルlist
    private RectTransform _rectTr;//Ui座標
    private Vector2 _maxRectTr = new Vector2(800, 450);//Ui座標の最大値
    private const float _valueChange = 0.5f;//カメラ座標の値を(-0.5~0.5)から(0~1)に変換
    #endregion
    ...
    #region 弾関連
    [Header("弾関連")]
    [SerializeField, Tooltip("弾の発射位置")] private Transform _shotPos;
    [SerializeField, Tooltip("発射位置を変更する時の範囲")] private Vector2[] _shotRotates;
    [SerializeField, Tooltip("連射のインターバル")] private float _shotIntervalTime;
    private float _shotIntervalCount = 0;//連射のインターバルのカウント
    ...
    private LockOnMotion _lockOnMotion = LockOnMotion.LOCK_ON;//ロックオンの状態
    ...
    private void Update()
    {
        ...
        if (Time.timeScale == 0 || _gameSystem.IsEvent() || _playerController._playerMotion == PlayerController.PlayerMotion.Death) { return; }
    ...
    private void SearchArray()
    {
        ...
        if (_playerStatus.MaxLockCount() || _playerController._playerMotion == PlayerController.PlayerMotion.Damage) { return; }
